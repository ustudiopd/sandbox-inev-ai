-- get_marketing_summary RPC 함수 수정
-- 문제: "aggregate function calls cannot be nested" 오류
-- 원인: jsonb_agg 안에서 count(*) 사용 (집계 함수 중첩)
-- 해결: 서브쿼리로 집계 후 jsonb_agg 적용
...
begin
  ...
  -- Source별 집계 (서브쿼리로 집계 후 jsonb_agg)
  select jsonb_agg(
    jsonb_build_object(
      'source', source,
      'count', cnt
  ...
  from (
    select 
      coalesce(utm_source, '__direct__') as source,
      count(*)::bigint as cnt
    from public.event_survey_entries e
    inner join public.event_survey_campaigns c on e.campaign_id = c.id
    where c.client_id = p_client_id
  ...
  from (
    select 
      coalesce(utm_medium, '__direct__') as medium,
      count(*)::bigint as cnt
    from public.event_survey_entries e
    inner join public.event_survey_campaigns c on e.campaign_id = c.id
    where c.client_id = p_client_id
  ...
  from (
    select 
      coalesce(utm_campaign, '__direct__') as campaign,
      count(*)::bigint as cnt
    from public.event_survey_entries e
    inner join public.event_survey_campaigns c on e.campaign_id = c.id
    where c.client_id = p_client_id
...
comment on function get_marketing_summary is '마케팅 캠페인 요약 데이터를 반환합니다 (Phase 1: Conversions 중심) - 집계 함수 중첩 오류 수정';

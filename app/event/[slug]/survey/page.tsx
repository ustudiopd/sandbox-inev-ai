...
export default async function PublicEventSurveyPage({ params, searchParams }: Props) {
  const { slug } = await params
  const q = await searchParams
  const supabase = createAdminSupabase()
  const { data: event, error } = await supabase
    .from('events')
    .select('id, slug, module_survey, module_utm')
    .eq('slug', slug)
    .limit(1)
    .single()

  if (error || !event) notFound()
  if (!event.module_survey) notFound()
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="mx-auto max-w-md rounded-lg border border-gray-200 bg-white p-8">
        <div className="mb-6">
          <h1 className="text-2xl font-semibold text-gray-900">설문</h1>
          <p className="mt-1 text-xs text-gray-500">{event.slug}</p>
        </div>
        <SurveyForm slug={event.slug} />

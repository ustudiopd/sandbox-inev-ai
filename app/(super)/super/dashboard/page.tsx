        ...
        <div className="mb-6 sm:mb-8">
          <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-4 mb-4">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm sm:text-base text-gray-600">전체 시스템 관리 및 모니터링</p>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-6 sm:mb-8">
          <div className="bg-white p-4 md:p-6 rounded-xl shadow-lg border border-gray-100">
            <div className="flex items-center justify-between mb-4 min-w-0">
              <h2 className="text-sm md:text-lg font-semibold text-gray-700 truncate">에이전시 수</h2>
              <div className="text-3xl md:text-4xl flex-shrink-0 ml-2">🏢</div>
            </div>
            <p className="text-2xl md:text-3xl lg:text-4xl font-bold text-blue-600">{agenciesCount || 0}</p>
            <Link
              href="/super/agencies"
          ...
          <div className="bg-white p-4 md:p-6 rounded-xl shadow-lg border border-gray-100">
            <div className="flex items-center justify-between mb-4 min-w-0">
              <h2 className="text-sm md:text-lg font-semibold text-gray-700 truncate">클라이언트 수</h2>
              <div className="text-3xl md:text-4xl flex-shrink-0 ml-2">👥</div>
            </div>
            <p className="text-2xl md:text-3xl lg:text-4xl font-bold text-purple-600">{clientsCount || 0}</p>
            <Link
              href="/super/clients"
          ...
          <div className="bg-white p-4 md:p-6 rounded-xl shadow-lg border border-gray-100">
            <div className="flex items-center justify-between mb-4 min-w-0">
              <h2 className="text-sm md:text-lg font-semibold text-gray-700 truncate">웨비나 수</h2>
              <div className="text-3xl md:text-4xl flex-shrink-0 ml-2">🎥</div>
            </div>
            <p className="text-2xl md:text-3xl lg:text-4xl font-bold text-green-600">{webinarsCount || 0}</p>
          </div>
        </div>
              ...
              {recentAgencies && recentAgencies.length > 0 ? (
                <ul className="space-y-2 sm:space-y-3">
                  {recentAgencies.map((agency) => (
                    <li
                      key={agency.id}
                      className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 sm:gap-0 p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors"
                    >
                      <div className="flex-1 min-w-0">
                        <span className="font-medium text-gray-800 block truncate">{agency.name}</span>
                      ...
                      <Link
                        href="/super/agencies"
                        className="text-blue-600 hover:underline text-sm font-medium w-full sm:w-auto text-center sm:text-left"
                      >
                        보기 →
                      </Link>
                    </li>

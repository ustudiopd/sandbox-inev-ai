...
/**
 * 캠페인 메타 썸네일 이미지 업로드
 * POST /api/event-survey/campaigns/[campaignId]/meta-thumbnail/upload
 */
export async function POST(
  ...
  try {
    ...
    // 파일명 생성 (campaignId-meta-thumbnail.확장자)
    const fileExtension = file.name.split('.').pop() || 'jpg'
    const filePath = `${campaignId}-meta-thumbnail.${fileExtension}`
    
    // Supabase Storage에 업로드 (webinar-thumbnails 버킷 재사용 또는 별도 버킷)

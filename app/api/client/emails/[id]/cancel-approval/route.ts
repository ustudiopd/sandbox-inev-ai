...
export async function POST(
  ...
  try {
    ...
    // ready 상태에서만 승인 취소 가능
    if (campaign.status !== 'ready') {
      return NextResponse.json(
        { success: false, error: `${campaign.status} 상태에서는 승인 취소할 수 없습니다` },
        { status: 400 }
      )
    }

    // 원자적 업데이트: ready → draft
    ...
    if (updateError || !updatedCampaign) {
      return NextResponse.json(
        { success: false, error: '승인 취소 실패: 상태가 변경되었습니다' },
        { status: 409 }
      )
    }
    ...
    await admin.from('audit_logs').insert({
      ...
      action: 'EMAIL_CAMPAIGN_CANCEL_APPROVAL',
    ...
    console.error('이메일 캠페인 승인 취소 오류:', error)
    return NextResponse.json(

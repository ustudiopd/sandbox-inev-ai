...
export default function ChatModeration({ webinarId }: ChatModerationProps) {
  ...
  const renderMessages = (customFontSize?: number) => {
    const size = customFontSize ?? fontSize
    return (
      <div className="space-y-2 max-h-[600px] overflow-y-auto" style={{ fontSize: `${size}px` }}>
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="font-semibold text-gray-800" style={{ fontSize: `${size}px` }}>
                    {message.user?.display_name || message.user?.email || '익명'}
                  </span>
                  <span className="text-gray-500" style={{ fontSize: `${size - 2}px` }}>
                    {formatTime(message.created_at)}
                  </span>
                  {message.hidden && (
                    <span className="bg-red-100 text-red-800 px-2 py-0.5 rounded" style={{ fontSize: `${size - 2}px` }}>숨김</span>
                  )}
                </div>
                <p className="text-gray-700 break-words" style={{ fontSize: `${size}px` }}>{message.content}</p>
              </div>

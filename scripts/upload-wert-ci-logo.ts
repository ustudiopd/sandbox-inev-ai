...
async function uploadWertCILogo() {
  try {
    const admin = createAdminSupabase()
    
    // 이미지 파일 읽기
    const imagePath = join(process.cwd(), 'img', 'wert', 'wert_CI.png')
    ...
    // 이미지 업로드 (기존 kewert_logo-w.png를 덮어쓰기)
    const filePath = 'wert/kewert_logo-w.png'

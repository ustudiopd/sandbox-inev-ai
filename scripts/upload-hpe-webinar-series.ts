...
async function uploadHPEWebinarSeries() {
  try {
    const admin = createAdminSupabase()
    
    // 이미지 파일 읽기
    const imagePath = join(process.cwd(), 'img', 'hpe', 'HPE_Webinar_Series.png')
    ...
    // 이미지 업로드 (hpe 폴더에 저장)
    const filePath = 'hpe/HPE_Webinar_Series.png'

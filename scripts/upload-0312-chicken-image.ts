...
async function upload0312ChickenImage() {
  try {
    const admin = createAdminSupabase()
    
    // 이미지 파일 읽기
    const imagePath = join(process.cwd(), 'img', 'wert', '0312_chicken.png')
    ...
    // 이미지 업로드
    const filePath = 'wert/0312_chicken.png'

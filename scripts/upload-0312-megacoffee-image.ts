...
async function upload0312MegacoffeeImage() {
  try {
    const admin = createAdminSupabase()
    
    // 이미지 파일 읽기
    const imagePath = join(process.cwd(), 'img', 'wert', '0312_megacoffee.png')
    ...
    // 이미지 업로드
    const filePath = 'wert/0312_megacoffee.png'

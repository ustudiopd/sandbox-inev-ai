...
async function checkAllRecentRegistrations() {
  const admin = createAdminSupabase()
  
  console.log('🔍 모든 최근 등록 확인 (CID 저장 여부)\n')
  ...
  if (!entries || entries.length === 0) {
    console.log('⚠️  최근 30분 내 등록이 없습니다.')
    console.log('\n전체 최근 등록 확인 중...\n')
    ...
    if (allEntries && allEntries.length > 0) {
      console.log(`✅ 전체 최근 등록 ${allEntries.length}건:\n`)
      allEntries.forEach((entry, index) => {
        const cidValue = entry.registration_data && typeof entry.registration_data === 'object' 
          ? (entry.registration_data as any).cid || (entry.registration_data as any).CID || null
          : null
        
        console.log(`${index + 1}. ${entry.name || 'N/A'} - ${new Date(entry.created_at).toLocaleString('ko-KR')}`)
        console.log(`   UTM: ${entry.utm_source || '없음'}`)
  ...
  console.log(`✅ 최근 30분 내 등록 ${entries.length}건 발견\n`)
  ...
  entries.forEach((entry, index) => {
    ...
    if (entry.registration_data) {
      console.log(`   Registration Data Keys: ${Object.keys(entry.registration_data).join(', ')}`)
    }
  ...
  console.log('=' .repeat(60))
  console.log('\n📊 CID 저장 통계\n')
  console.log(`   최근 30분 내 등록: ${entries.length}건`)
...
checkAllRecentRegistrations().catch(console.error)

...
## 1. 현재 집중하고 있는 작업  
- **작업명**: 사이드바 개선 및 통계 기능 명세서 작성
- **목표**: 
  - 운영콘솔 페이지에서 사이드바 표시되도록 수정
  - 기능별 통계 구현 명세서 작성 완료
  - 각 기능(채팅, Q&A, 폼, 추첨, 파일, 등록자, 접속)별 상세 통계 설계
- **상태**: ✅ 완료
  - 운영콘솔 페이지(`/webinar/[id]/console`)에서 사이드바 표시되도록 LayoutWrapper 수정 완료
  - 정규식 개선으로 한글 slug 포함 URL도 정상 매칭되도록 수정
  - 기능별 통계 구현 명세서 작성 완료 (`docs/feature-statistics-implementation-spec.md`)
  - 8개 기능별 통계 항목, API 설계, SQL 쿼리 예시, UI 컴포넌트 구조 포함

## 2. 다음 예정 작업  
- **우선순위 높음**: 
  1. 기능별 통계 구현 (Phase 1: 기본 통계 강화)
  ...
  2. 웨비나 등록 페이지 구현 (`/webinar/[id]/register`)
  3. 초대 링크 처리 (`/invite/[token]`)

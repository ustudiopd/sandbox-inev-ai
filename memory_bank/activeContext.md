...
## 1. 현재 집중하고 있는 작업  
- **작업명**: 사이드바 및 레이아웃 개선 완료
- **목표**: 
  - 사이드바 접기 기능 제거 (항상 펼쳐진 상태 유지)
  - 운영콘솔 레이아웃 개선 (사이드바와 콘텐츠 사이 회색 공간 제거, 가운데 정렬)
  - 대시보드와 운영콘솔 레이아웃 일관성 유지
- **상태**: ✅ 완료
  - 사이드바 접기/펼치기 버튼 완전 제거
  - LayoutWrapper에서 main 태그 너비를 100vw 기준으로 계산하여 콘텐츠 가운데 정렬
  - 중복 레이아웃 래핑 제거 (`app/(admin)/layout.tsx`)
  - 배경색을 LayoutWrapper의 main 태그로 이동하여 일관성 유지
  - 대시보드와 운영콘솔 모두 동일한 레이아웃 구조 적용

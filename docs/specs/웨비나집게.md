...
* “한 번 입장” = “세션 1행”
* ~~heartbeat는 **insert가 아니라 update**~~ (Phase 1에서 구현 예정)
* 현재는 입장/퇴장 시간만 기록 (heartbeat 기반 누적은 Phase 1에서 추가 예정)
...
* 식별/관계
  ...
  * `webinar_id` (FK) ✅
  * `user_id` (FK, nullable - 게스트 지원) ✅
  * `session_id` (클라이언트 생성, 재시도/중복 방지용) ✅
* 시간

  * `entered_at` (세션 시작) ✅
  * ~~`last_heartbeat_at` (마지막 heartbeat)~~ (Phase 1에서 추가 예정)
  * `exited_at` (세션 종료, 없으면 열려있는 상태) ✅
* 누적(서버 계산)

  * `duration_seconds` (exited_at - entered_at, 트리거로 자동 계산) ✅
  * ~~`watched_seconds_raw` (heartbeat Δt 누적)~~ (Phase 1에서 추가 예정)
  * ~~`closed_reason`~~ (Phase 2에서 추가 예정)
* 메타(필요 최소)

  * `user_agent` ✅
  * `referrer` ✅
  * `ip_address` ✅ (프록시 헤더 고려)
* 멀티테넌시
...
**✅ 부분 구현 완료 (2026-02-06)**
* ✅ `webinar_user_sessions` 마이그레이션 완료
* ✅ `/api/webinars/[webinarId]/access/track`에 세션 insert 추가
* ✅ `/api/webinars/[webinarId]/access/exit` API 생성
* ✅ 클라이언트에서 퇴장 이벤트 처리 (`beforeunload`, `navigator.sendBeacon`)

**⏳ 남은 작업 (Phase 1 완료를 위해)**
* ⏳ `presence/ping`에 `session_id`를 받아 heartbeat update 동작 추가
* ⏳ `last_heartbeat_at`, `watched_seconds_raw` 필드 추가
* ⏳ 클라이언트에서 heartbeat 스케줄러 구현 (지터 포함)
* ⏳ visibility 기반으로 ping 제어(최소 정책 1 적용)

**현재 상태**
* 한 사용자가 여러 번 들어오면 `webinar_user_sessions`에 세션이 누적됨 ✅
* 입장/퇴장 시간이 기록되어 `duration_seconds`가 자동 계산됨 ✅
* 통계 화면에서 "입장횟수/총 체류시간"이 계산 가능함 ✅
* ⏳ `watched_seconds_raw`는 아직 heartbeat 기반 누적 미구현
